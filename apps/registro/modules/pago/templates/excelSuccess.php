...
if($excel)
{
        ...
        $objSheet->getCell('D1')->setValue('Pagos (Bs.)');
        $objSheet->getCell('E1')->setValue('Deuda');
...
//    $conmutador= 1;
    foreach ($excel as $datos)
    {
        ...
        //CALCULO DE DEUDA
        //CALCULO DE PAGO
        $disciplinas= Doctrine::getTable('Operaciones_RegistroDisciplina')->findByRegistroIdAndStatus($datos->getRegistro(), 'A');
        ...
        $objSheet->getCell('D'.$row_index)->setValue(number_format($field4, 2, ',', '.'));
        $objSheet->getCell('E'.$row_index)->setValue(number_format($deuda, 2, ',', '.'));

        $objSheet->getStyle('E'.$row_index)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
        $objSheet->getStyle('D'.$row_index)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
