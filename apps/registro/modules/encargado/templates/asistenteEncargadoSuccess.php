<?php include(sfConfig::get("sf_root_dir").'/apps/registro/modules/encargado/lib/validator_asistente_encargado.php'); ?>

<script>
    function guardar_encargado(destination){
        $('#add_or_next').val(destination);
        
        $.ajax({
            url:'<?php echo sfConfig::get('sf_app_registro_url'); ?>encargado/saveEncargado',
            type:'POST',
            dataType:'html',
            data: $('#form_encargado').serialize(),
            beforeSend: function(Obj){
                $('#save_encargado_msj').html('<?php echo image_tag('icon/cargando.gif', array('style'=>'width: 16px; vertical-align: middle')); ?> Guardando encargado...');
            },
    ...
    function buscarEncargado() {
        ...
        if(cedula === '') {
            ...
            $.ajax({
                url:'<?php echo sfConfig::get('sf_app_registro_url'); ?>encargado/buscarEncargado',
                type:'POST',
                dataType:'json',
                data:'cedula='+cedula+'&regid='+registro_id,
                beforeSend: function(Obj){
                    $('#buscar_encargado_msj').html('<?php echo image_tag('icon/cargando.gif', array('style'=>'width: 16px; vertical-align: middle')); ?> Buscando encargado ...');
                },
                success:function(json, textStatus){
                    if(json.status === 'ok') {
                        $('#buscar_encargado_msj').html('');
                        ...
                        if(json.status === 'existe') {
                            $("#buscar_encargado_msj").html('<x id="inner_msj"><font style="color: red">La persona ya se encuentra inscrita como encargado en esta misma empresa</font></x>');
                            setTimeout(function(){
                                $("#inner_msj").fadeOut(1000);
                            }, 6000);
                            
                            $('.to_hid_encargado').hide('fast');
                        }else {
                            $('.to_hid_encargado').show('fast');
                            ...
                            if(json.status === 'saime') {
                                ...
                                $("#buscar_encargado_msj").html('<x id="inner_msj"><font style="color: green">Por favor complete los datos faltantes</font></x>');
                                setTimeout(function(){
                                    $("#inner_msj").fadeOut(1000);
                                }, 3000);
                            }else {
                                $("#buscar_encargado_msj").html('<x id="inner_msj"><font style="color: green">La persona no esta registrada aún</font></x>');
                                setTimeout(function(){
                                    $("#inner_msj").fadeOut(1000);
                                }, 3000);
                            }
...
<form id="form_encargado">
        ...
        <div class="sf_admin_form_row">
            <div>
                ...
                <div class="content">
                    ...
                    <a href="#" onClick="javascript: buscarEncargado(); return false;"><?php echo image_tag('icon/2execute.png', array('style'=>'vertical-align: middle')) ?></a>
                    ...
                    <x id="buscar_encargado_msj"></x>
                </div>
                <div class="help">Indique n&uacute;mero de documento de identidad</div>
            </div>
        </div>
        <div class='to_hid_encargado' style='display: none'>
            <div class="sf_admin_form_row">
                <div>
                    <label>Nombre</label>
                    <div class="content">
            ...
            <div class="sf_admin_form_row">
                <div>
                    <label>Cargo</label>
                    <div class="content">
                        <input type='text' maxlength='40' id='operaciones_registro_encargado_cargo' name='operaciones_registro_encargado[cargo]' value='' />
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class='to_hid_encargado' style='display: none'>
        ...
        <fieldset>
            <div class="sf_admin_form_row">
                <button id="guardar_encargado_add" onClick='javascript: $("#add_or_next").val("2")' style="height: 35px">
                    <strong>Guardar y crear otro</strong>&nbsp;<?php echo image_tag('icon/add.png', array('style' => 'vertical-align: middle')) ?>
                </button>&nbsp;&nbsp;
                <button id="guardar_encargado_next" onClick='javascript: $("#add_or_next").val("3")' style="height: 35px">
                    <strong>Guardar y continuar</strong>&nbsp;<?php echo image_tag('icon/execute.png', array('style' => 'vertical-align: middle')) ?>
                </button>
                <input type='hidden' name='destination' id='add_or_next' value='3' />
                <x id="save_encargado_msj"></x>
            </div>
        </fieldset>
    </div>
</form>